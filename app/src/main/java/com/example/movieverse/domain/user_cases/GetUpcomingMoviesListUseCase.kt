...
import com.example.movieverse.data.model.toUpcomingMoviesModel
...
class GetUpcomingMoviesListUseCase @Inject constructor(private val moviesRepository: MoviesRepository) {
    ...
    operator fun invoke(): Flow<ViewState<MoviesModel>> = flow {
        ...
        try {
            ...
            val responseModel =
                if (response.results.isEmpty()) emptyList<MoviesModel.Result>() else response.results.map { it.toUpcomingMoviesModel() }
